...
class firstconv3x3(nn.Module):
    def __init__(self, inp, oup, stride,rep_beta):
...
class SqueezeAndExpand(nn.Module):
    def __init__(self, channels, planes, ratio = 8, attention_mode = "hard_sigmoid",rep_beta=1):
...
class Attention(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, drop_rate = 0.1, infor_recoupling=True, groups = 1,rep_beta=1):
        ...
        if self.infor_recoupling:
            self.se = SqueezeAndExpand(planes, planes, attention_mode = "sigmoid",rep_beta=rep_beta)
...
class FFN_3x3(nn.Module):
    def __init__(self, inplanes, planes, stride = 1, downsample = None, drop_rate = 0.1, infor_recoupling = True, groups = 1,rep_beta=1):
        ...
        if self.infor_recoupling:
            self.se = SqueezeAndExpand(inplanes, planes, attention_mode = "sigmoid",rep_beta=rep_beta)
            self.scale = nn.Parameter(torch.ones(1, planes*rep_beta, 1, 1)*0.5)
        self.rep_beta=rep_beta
...
class FFN_1x1(nn.Module):
    def __init__(self, inplanes, planes, stride = 1, attention = True, drop_rate = 0.1, infor_recoupling = True,rep_beta=1):
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, drop_rate = 0.1, mode = "scale",rep_beta=1):
        super(BasicBlock, self).__init__()
        self.inplanes = inplanes
        self.planes = planes
        
        if mode == "scale":
            self.Attention = Attention(inplanes, inplanes, stride, None, drop_rate = drop_rate, groups = 1,rep_beta=rep_beta)
        else:
            self.Attention = FFN_3x3(inplanes, inplanes, stride, None, drop_rate = drop_rate, groups = 1,rep_beta=rep_beta)
        
        if inplanes == planes:
          self.FFN = FFN_1x1(inplanes, inplanes, drop_rate = drop_rate,rep_beta=rep_beta)
                        
        else:
          self.FFN_1 = FFN_1x1(inplanes, inplanes, drop_rate = drop_rate,rep_beta=rep_beta)

          self.FFN_2 = FFN_1x1(inplanes, inplanes, drop_rate = drop_rate,rep_beta=rep_beta)
...
class BNext(nn.Module):
    def __init__(self, num_classes=1000, size = "tiny", ELM_Attention = True, Infor_Recoupling = True,rep_beta=2):
        ...
        for i in range(len(stage_out_channel)):
            if i == 0:
                self.feature.append(firstconv3x3(3, stage_out_channel[i], 1 if num_classes != 1000 else 2,rep_beta=rep_beta))
            elif i == 1:
                self.feature.append((basicblock(stage_out_channel[i-1], stage_out_channel[i], 1, drop_rate = drop_rates[i], mode = "bias",rep_beta=rep_beta)))
            elif stage_out_channel[i-1] != stage_out_channel[i] and stage_out_channel[i] != stage_out_channel[1]:
                self.feature.append(basicblock(stage_out_channel[i-1], stage_out_channel[i], 2, drop_rate = drop_rates[i], mode = "scale" if i%2 == 0 else "bias",rep_beta=rep_beta))
            else:
                self.feature.append(basicblock(stage_out_channel[i-1], stage_out_channel[i], 1, drop_rate = drop_rates[i], mode = "scale" if i%2 == 0 else "bias",rep_beta=rep_beta))
