...
class SqueezeAndExpand(nn.Module):
    def __init__(self, channels, planes, ratio = 8, attention_mode = "hard_sigmoid",rep_beta=1):
        ...
        self.se = nn.Sequential(
                        ...
                        nn.Conv2d(channels*rep_beta, channels // ratio, kernel_size = 1, padding = 0),
                        nn.ReLU(channels//ratio),
                        nn.Conv2d(channels//ratio, planes*rep_beta, kernel_size = 1, padding = 0),
                    )
...
class BNext(nn.Module):
    def __init__(self, num_classes=1000, size = "tiny", ELM_Attention = True, Infor_Recoupling = True,rep_beta=2):
        ...
        self.prelu = nn.PReLU(stage_out_channel[-1]*rep_beta)
        self.pool1 = nn.AdaptiveAvgPool2d(1)
        self.fc = nn.Linear(stage_out_channel[-1]*rep_beta, num_classes)
