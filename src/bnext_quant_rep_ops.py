...
class firstconv3x3(nn.Module):
    def __init__(self, inp, oup, stride, quant = False,rep_beta=2):
        ...
        self.bn1 = nn.BatchNorm2d(oup*rep_beta)
        self.prelu = nn.PReLU(oup*rep_beta, oup*rep_beta)
        self.rep_beta=rep_beta

    def forward(self, x):
        out = self.conv1(x)
        out = self.bn1(out)
        out = out.repeat(1,self.rep_beta,1,1)
        return out
...
class SqueezeAndExpand(nn.Module):
    def __init__(self, channels, planes, ratio=8, attention_mode="hard_sigmoid", quant = False, bits = 8,rep_beta=1):
        super(SqueezeAndExpand, self).__init__()
        if not quant:
            self.se = nn.Sequential(
                nn.AdaptiveAvgPool2d((1,1)),
                nn.Conv2d(channels*rep_beta, int((channels // ratio)/rep_beta), kernel_size = 1, padding = 0),
                LambdaLayer(lambda x:x.repeat(1,rep_beta*rep_beta,1,1)),
                nn.ReLU(channels//ratio*rep_beta),
                nn.Conv2d(channels//ratio*rep_beta, int(planes/rep_beta), kernel_size = 1, padding = 0),
                LambdaLayer(lambda x:x.repeat(1,rep_beta*rep_beta,1,1)),
            )
            ...
            self.se = nn.Sequential(
                nn.AdaptiveAvgPool2d((1, 1)),
                QuantizeConv(channels*rep_beta, int((channels // ratio)/rep_beta), 1, 1, 0, 1, 1, True, activation_bits=bits, weight_bits=bits),
                LambdaLayer(lambda x:x.repeat(1,rep_beta*rep_beta,1,1)),
                nn.ReLU(),
                QuantizeConv(channels // ratio*rep_beta, int(channels/rep_beta), 1, 1, 0, 1, 1, True, activation_bits=bits, weight_bits=bits),
                LambdaLayer(lambda x:x.repeat(1,rep_beta*rep_beta,1,1)),
...
class Attention(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, drop_rate=0.1, groups=1, quant = False, se_bits = 32):
        ...
        self.move = LearnableBias(inplanes*rep_beta)
        self.binary_activation = HardSign(range=[-1.5, 1.5])
        self.binary_conv = HardBinaryConv(inplanes*rep_beta, int(planes/rep_beta), kernel_size=3, stride=stride, groups=groups)

        self.norm1 = nn.BatchNorm2d(planes*rep_beta)
        self.norm2 = nn.BatchNorm2d(planes*rep_beta)

        self.activation1 = nn.PReLU(inplanes*rep_beta)
        self.activation2 = nn.PReLU(planes*rep_beta)
        ...
        self.se = SqueezeAndExpand(planes, planes, attention_mode="sigmoid", quant=quant, bits = se_bits,rep_beta=rep_beta)
        self.scale = nn.Parameter(torch.ones(1, planes*rep_beta, 1, 1) * 0.5)
        self.rep_beta=rep_beta
    ...
    def forward(self, input):
        ...
        x = x.repeat(1,self.rep_beta*self.rep_beta,1,1)
        x = self.norm1(x)
...
class FFN_3x3(nn.Module):
    def __init__(self, inplanes, planes, stride=1, downsample=None, drop_rate=0.1, groups=1, quant = False, se_bits = 32,rep_beta=1):
        ...
        self.se = SqueezeAndExpand(inplanes, planes, attention_mode="sigmoid", quant=quant, bits = se_bits,rep_beta=rep_beta)
        self.scale = nn.Parameter(torch.ones(1, planes*rep_beta, 1, 1) * 0.5)
        self.rep_beta=rep_beta
...
class FFN_1x1(nn.Module):
    def __init__(self, inplanes, planes, stride=1, drop_rate=0.1, quant = False, se_bits = 32,rep_beta=1):
        ...
        self.move = LearnableBias(inplanes*rep_beta)
        self.binary_activation = HardSign(range=[-1.5, 1.5])
        self.binary_conv = HardBinaryConv(inplanes*rep_beta, int(planes/rep_beta), kernel_size=1, stride=stride, padding=0)
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, drop_rate=0.1, mode="scale", quant = False, se_bits = 32,rep_beta=1):
        super(BasicBlock, self).__init__()
        self.inplanes = inplanes
        self.planes = planes

        if mode == "scale":
            self.Attention = Attention(inplanes, inplanes, stride, None, drop_rate=drop_rate, groups=1, quant = quant, se_bits = se_bits,rep_beta=rep_beta)
        else:
            self.Attention = FFN_3x3(inplanes, inplanes, stride, None, drop_rate=drop_rate, groups=1, quant = quant, se_bits = se_bits,rep_beta=rep_beta)

        if inplanes == planes:
            self.FFN = FFN_1x1(inplanes, inplanes, drop_rate=drop_rate, quant = quant, se_bits = se_bits,rep_beta=rep_beta)

        else:
            self.FFN_1 = FFN_1x1(inplanes, inplanes, drop_rate=drop_rate, quant = quant, se_bits = se_bits,rep_beta=rep_beta)

            self.FFN_2 = FFN_1x1(inplanes, inplanes, drop_rate=drop_rate, quant = quant, se_bits = se_bits,rep_beta=rep_beta)
...
class BNext(nn.Module):
    def __init__(self, num_classes=1000, size="tiny", quant = False, se_bits = 32,rep_beta=2):
        ...
        for i in range(len(stage_out_channel)):
            if i == 0:
                self.feature.append(firstconv3x3(3, stage_out_channel[i], 1 if num_classes != 1000 else 2, quant = quant,rep_beta=rep_beta))
            elif i == 1:
                self.feature.append((BasicBlock(stage_out_channel[i - 1], stage_out_channel[i], 1,
                                                drop_rate=drop_rates[i], mode="bias", quant = quant, se_bits = se_bits,rep_beta=rep_beta)))
                ...
                self.feature.append(
                    BasicBlock(stage_out_channel[i - 1], stage_out_channel[i], 2, drop_rate=drop_rates[i],
                               mode="scale" if i % 2 == 0 else "bias", quant = quant, se_bits = se_bits,rep_beta=rep_beta))
            else:
                self.feature.append(
        ...
        self.final_c=stage_out_channel[-1]

    def forward(self, x):
        for i, block in enumerate(self.feature):
            x = block(x)
        x = self.prelu(x[:,:self.final_c])
        x = self.pool1(x)
        x = x.view(x.size(0), -1)
        x = self.fc(x)
